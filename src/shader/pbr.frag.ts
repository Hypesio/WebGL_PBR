...
vec3 pbr_color(vec3 viewDirection, vec3 lightDirection, float roughness, float metallic, vec3 albedo) {
  ...
  float D = rough2 / (M_PI * pow(pow(max(dot(vNormalWS, h), 0.0), 2.0) * (rough2 - 1.0) + 1.0, 2.0)); 
  // Fresnel
  vec3 f0 = vec3(0.04);
  f0 = mix(f0, albedo, metallic);
  vec3 F = f0 + (1.0 - f0) * pow(1.0 - dot(lightDirection, vNormalWS), 5.0);
  // G
...
main()
{
  ...
  vec3 color = pbr_color(viewDirection, lightDirection, uMaterial.roughness, uMaterial.metallic, albedo);
